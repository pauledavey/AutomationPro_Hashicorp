...
sleep 1 & spinner $! "Install wget, nano, git & unzip"
sudo yum install wget nano unzip -y > /dev/null 2>&1

sleep 1 & spinner $! "Updating OS and components"
sudo yum update -y > /dev/null 2>&1

sleep 1 & spinner $! "Create the /usr/bin/hashicorp folder"
mkdir -p /usr/local/bin/hashicorp > /dev/null 2>&1

sleep 1 & spinner $! "Create the /usr/bin/hashicorp/packer folder"
mkdir -p /usr/local/bin/hashicorp/packer > /dev/null 2>&1

sleep 1 & spinner $! "Navigate to directory"
cd /usr/local/bin/hashicorp/packer > /dev/null 2>&1

sleep 1 & spinner $! "Download Packer [version 1.6.2]"
wget https://releases.hashicorp.com/packer/1.6.2/packer_1.6.2_linux_amd64.zip > /dev/null 2>&1

sleep 1 & spinner $! "Extract Packer from zip file"
unzip packer_1.6.2_linux_amd64.zip > /dev/null 2>&1

sleep 1 & spinner $! "Removing zip file"
rm -f packer_1.6.2_linux_amd64.zip > /dev/null 2>&1

sleep 1 & spinner $! "Confirm correct version of Hashicorp Packer is installed"
packerversion=$(/usr/local/bin/hashicorp/packer/packer --version) > /dev/null 2>&1

if [ "$packerversion" == "1.6.2" ]; then
	sleep 1 & spinner $! "Packer [version 1.6.2] install. They call it a Royale with cheese..."
else
...
sleep 1 & spinner $! "Cloning Automationpro public packer github repo"
cd /usr/local/bin/hashicorp/packer > /dev/null 2>&1
